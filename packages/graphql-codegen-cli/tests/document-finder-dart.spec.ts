...
import { extractDocumentStringFromDartCodeFile } from '../src/utils/document-finder-dart';

describe('extractDocumentStringFromDartCodeFile', () => {
  ...
  function extractDart(fileName: string) {
    const fileContent = fs.readFileSync(`./tests/test-files/${fileName}`).toString();
    const doc = extractDocumentStringFromDartCodeFile(fileContent);
    expect(tryParse(doc)).not.toThrow();
    return doc.trim();
  }
  ...
  it('file with gql and template literal', () => {
    ...
    const dart = extractDart('dart1.dart');
